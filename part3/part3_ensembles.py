...
for name, model in cv_models.items():
    # n_jobs=1 (not -1): on some Windows + Python 3.13 setups, joblib's
    # multiprocessing backend crashes with "ModuleNotFoundError: No module
    # named '_posixsubprocess'" (a POSIX-only module being pulled in by the
    # worker-process bootstrap on Windows). Running single-process sidesteps
    # that bug entirely; it's slower but always works. If you're on Linux/Mac
    # or have confirmed n_jobs=-1 works in your environment, it's safe to
    # change this back for speed.
...
grid_search = GridSearchCV(
    ...
    scoring="roc_auc", n_jobs=1,  # n_jobs=1: see note in Task 5 (avoids a Windows/Python 3.13 joblib multiprocessing bug)
)
